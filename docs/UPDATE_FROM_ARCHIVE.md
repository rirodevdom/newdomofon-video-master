...
Архив скачивается вне сервера. После передачи файла на master:
...
Имя распакованной папки может отличаться. Запускайте updater из корня распакованного master-проекта.
...
Сервер, база и сервисы при этом не изменяются. Скрипт также вычисляет SHA-256 отпечаток содержимого архива.
