...
for command in node npm python3 rsync tar sha256sum mktemp pg_dump; do
...
[[ "$(manifest_value project_type)" == "master" ]] ||
  fail "Пакет предназначен не для video master"
...
TEMP_ROOT="$(mktemp -d "/var/tmp/newdomofon-master-offline-${STAMP}-XXXXXX")"
tar -xzf "$CACHE_ARCHIVE" -C "$TEMP_ROOT"
[[ -d "$TEMP_ROOT/npm-cache/_cacache" ]] ||
  fail "В offline-пакете отсутствует npm cache content store"

# Cache нужен только deploy-процессу. Убираем payload из source tree, чтобы
# штатный rsync не переносил большой архив в /opt/newdomofon-video-master.
...
log "Offline update master завершён успешно"
