...
def replace_one_of(text: str, candidates: list[tuple[str, str]], label: str) -> str:
    for old, new in candidates:
        if new in text:
            return text
        if text.count(old) == 1:
            return text.replace(old, new, 1)
    counts = [text.count(old) for old, _new in candidates]
    raise RuntimeError(f"{label}: no supported source fragment found, counts={counts}")
...
'''
  ...
  const rewritten = scope === 'archive'
    ...
    legacy_old = "    { method: 'POST', body: JSON.stringify({ start: params.start, end: params.end }) },"
    legacy_new = "    { method: 'POST', body: JSON.stringify({\n      start: timelineIsoToNodeArchiveIso(params.start),\n      end: timelineIsoToNodeArchiveIso(params.end)\n    }) },"
    text = replace_one_of(
        text,
        [(viewer_old, viewer_new), (legacy_old, legacy_new)],
    ...
    status_old = "    upstream_media_auth: 'agent-token-hash'\n  });"
    ...
    for required in (
        ...
        "sendProxyHlsPlaylist(upstream, res, browserToken, scope)",
    ):
