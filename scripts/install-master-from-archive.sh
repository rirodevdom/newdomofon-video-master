...
find_source_root() {
  local search_root="$1"
  local helper
  helper="$(find "$search_root" -type f \
...
if [[ -n "$SOURCE_ARCHIVE" ]]; then
  ...
  echo "Using already extracted local sources: $SOURCE_ROOT"
else
  ...
  [[ -n "$SOURCE_ARCHIVE" ]] || {
    echo "No project archive was found in /root" >&2
    exit 66
  }
...
git add -A
git commit -q -m 'Local archive installation source'
LOCAL_COMMIT="$(git rev-parse HEAD)"

if [[ -e "$PROJECT_DIR" ]]; then
  OLD_PROJECT_BACKUP="${PROJECT_DIR}.before-local-archive-${STAMP}"
...
if [[ -d "$PROJECT_DIR/.git" ]]; then
  ...
  printf '%s\n' "local-archive-commit=${LOCAL_COMMIT}" \
    >"$PROJECT_DIR/.installed-from-local-archive"
fi

if (( RC != 0 )); then
  echo
  echo "Local archive installation failed with exit code $RC" >&2
...
echo
echo "LOCAL ARCHIVE MASTER INSTALLATION COMPLETED"
echo "Source archive: ${SOURCE_ARCHIVE:-already extracted sources}"
