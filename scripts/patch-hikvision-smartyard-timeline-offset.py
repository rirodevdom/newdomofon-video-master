...
def patch_gateway(path: Path) -> None:
    ...
    text = replace_once(
        ...
        "    const startMs = HIK_TIMELINE_OFFSET.timelineToInternalMs(Date.parse(queryStart));\n    const endMs = HIK_TIMELINE_OFFSET.timelineToInternalMs(Date.parse(queryEnd));",
        'query archive window conversion',
    )
    text = replace_once(
        text,
        ...
        'path archive window conversion',
    )
    text = replace_once(
        text,
        "  if (match) return { startMs: Number(match[1]) * 1000, endMs: Date.now() };",
        "  if (match) return { startMs: HIK_TIMELINE_OFFSET.timelineToInternalMs(Number(match[1]) * 1000), endMs: Date.now() };",
        'absolute timeshift conversion',
    )
    text = replace_once(
        text,
        ...
        'recording status incoming conversion',
    )
    text = replace_once(
        text,
        "    const ranges = value.ranges.filter((item) => item.from + item.duration >= requestedFrom);",
        "    const ranges = value.ranges\n      .filter((item) => item.from + item.duration >= requestedFrom)\n      .map((item) => ({ ...item, from: Math.floor(HIK_TIMELINE_OFFSET.internalToTimelineMs(item.from * 1000) / 1000) }));",
        'recording status outgoing conversion',
    )
    text = replace_once(
        text,
        "      `#EXT-X-PROGRAM-DATE-TIME:${new Date(startMs).toISOString()}` ,".replace(' ` ,', '`,'),
        "      `#EXT-X-PROGRAM-DATE-TIME:${HIK_TIMELINE_OFFSET.timelineIso(startMs)}` ,".replace(' ` ,', '`,'),
        'program date time conversion',
    )
    text = replace_once(
        text,
        "      const targetMs = targetSec * 1000;",
        "      const targetMs = HIK_TIMELINE_OFFSET.timelineToInternalMs(targetSec * 1000);",
        'preview conversion',
    )
    text = replace_once(
        text,
        ...
        "  const endMs = endRaw ? HIK_TIMELINE_OFFSET.timelineToInternalMs(parseMoment(endRaw)) : Date.now();\n  const startMs = startRaw ? HIK_TIMELINE_OFFSET.timelineToInternalMs(parseMoment(startRaw)) : endMs - EVENT_DEFAULT_HOURS * 3600000;",
        'event incoming conversion',
    )
    text = replace_once(
        text,
        "    const timestamp = Date.parse(String(item.occurred_at || item.created_at || ''));\n    if (!Number.isFinite(timestamp)) continue;",
        "    const timestamp = Date.parse(String(item.occurred_at || item.created_at || ''));\n    if (!Number.isFinite(timestamp)) continue;\n    const timelineTimestamp = HIK_TIMELINE_OFFSET.internalToTimelineMs(timestamp);",
    ...
    text = replace_once(
        ...
        "        internal_secret_configured: Boolean(INTERNAL_SECRET),\n        hikvision_timeline_offset_seconds: HIK_TIMELINE_OFFSET_SECONDS",
        'health offset marker',
    )

    if MARKER not in text or 'HIK_TIMELINE_OFFSET.timelineToInternalMs' not in text:
        raise RuntimeError('Hikvision timeline offset markers incomplete')
    path.write_text(text, encoding='utf-8')
    print('Hikvision SmartYard timeline now subtracts 10800 seconds by default to match DVR wall-clock labels')
