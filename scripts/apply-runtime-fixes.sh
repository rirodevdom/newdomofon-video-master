...
Documentation=file:/opt/newdomofon-video-master/docs/BAREMETAL_DEBIAN12.md
...
User=newdomofon
Group=newdomofon
WorkingDirectory=/opt/newdomofon-video-master/public-events-proxy
EnvironmentFile=/etc/newdomofon-video/app.env
Environment=NODE_ENV=production
Environment=PUBLIC_EVENTS_PORT=3057
ExecStart=/usr/bin/node /opt/newdomofon-video-master/public-events-proxy/server.js
Restart=always
RestartSec=3
TimeoutStopSec=20
KillSignal=SIGTERM
...
User=newdomofon
Group=newdomofon
WorkingDirectory=/opt/newdomofon-video-master/smartyard-compat-proxy
EnvironmentFile=/etc/newdomofon-video/app.env
Environment=NODE_ENV=production
Environment=SMARTYARD_COMPAT_PORT=3082
Environment=DVR_ENGINE_URL=http://127.0.0.1:3010
ExecStart=/usr/bin/node /opt/newdomofon-video-master/smartyard-compat-proxy/server.js
Restart=always
RestartSec=3
TimeoutStopSec=20
KillSignal=SIGTERM
