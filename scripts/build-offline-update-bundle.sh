...
prepare_source() {
  PROJECT_DIR="$STAGE" bash "$STAGE/scripts/prepare-master-runtime-source.sh"
  if [[ -f "$STAGE/scripts/patch-system-managed-token-ui.py" ]]; then
    python3 "$STAGE/scripts/patch-system-managed-token-ui.py" --project-dir "$STAGE"
  fi
}
